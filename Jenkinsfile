pipeline {
    agent any
    environment {
        FLYWAY_URL = 'jdbc:mysql://172.174.141.97:32005/ecommerce?SSL=false&allowPublicKeyRetrieval=true'
        FLYWAY_USER = 'root'
    ...
    stages {
        ...
        stage('database migration'){
            steps{
             sh """
             flyway -url=$FLYWAY_URL -user=$FLYWAY_USER -password=$FLYWAY_PASSWORD -locations=$FLYWAY_LOCATIONS -cleanDisabled="false" clean
